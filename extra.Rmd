...
ggplot(distance_r, aes(x = larval_generation, y = mean, group = group)) +
  geom_rect(data = distance_r, aes(x = larval_generation, y = mean), 
            xmin = as.numeric(distance_r$larval_generation[[1]]) - 1.8, 
            xmax = as.numeric(distance_r$larval_generation[[3]]) + 1.8, 
            ymin = 9.944165-0.5209383,
            ymax = 9.944165+0.5209383,
            fill = "grey85",
            alpha = 0.3) +
    geom_rect(data = distance_r, aes(x = larval_generation, y = mean), 
            xmin = as.numeric(distance_r$larval_generation[[1]]) - 1.8, 
            xmax = as.numeric(distance_r$larval_generation[[3]]) + 1.8, 
            ymin = 5.883139-0.5088491,
            ymax = 5.883139+0.5088491,
            fill = "grey85",
            alpha = 0.3) +
  geom_hline(yintercept = 9.944165, color = "deepskyblue2", size = 3) +
  geom_hline(yintercept = 5.883139, color = "firebrick3", size = 3) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se, color = group), position = position_dodge(width = 0.3), width = 0.2, linewidth = 2) +
  geom_point(aes(x = larval_generation, y = mean, group = group, fill = group), position = position_dodge(width = 0.3), size = 7, pch = 22, stroke = 2, color = "black") +
  theme_set(theme_cowplot(12)) +
  ...
  theme(legend.position = c(0.03,0.91)) + 
...
##### 1a. Test fit of distance, speed, and acceleration data to a normal distribution; run the Rmd chunk below.

##### Test fit of distance, speed, and acceleration data to normal distribution:
```{r}
# load mobility data
load("data/mobility.Rda")
...
Given that all pairwise metrics are highly correlated, we opted to use only a single metric as a proxy for larval mobility, i.e., total distance walked. All downstream analyses were conducted for only these data.
...
distance_accel <- ggplot(mobility, aes(x = total_distance, y = ave_accel)) + 
  ...
  theme(axis.title.x = element_text(vjust = -0.5)) +
  theme(axis.text.x = element_text(hjust = 0.5)) +
  labs(x = "Total distance walked (mm)") +
...
distance_speed + distance_accel + speed_accel + plot_layout(nrow = 1, ncol = 3)
