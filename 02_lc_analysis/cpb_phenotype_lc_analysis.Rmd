...
In this step, we read in the raw data file (which includes some sample metadata, described below; `../raw_data/lc_data.xlsx`), manipulate and clean the data.
...
lc <- read_excel("../raw_data/lc_data.xlsx")
...
jpeg("figures/survival.jpeg", width = 10000, height = 8000, res = 1100)

ggplot(lc_averages, aes(x = dosage, y = mean)) + 
  geom_bar(stat = "identity", position = position_dodge(), fill = "grey", color = "black", linewidth = 1) +
  geom_text(aes(label = paste(format(round(mean*100, digits = 2), nsmall =2), "%"), vjust = -0.6, hjust = 0.5, size = 4), show.legend = F) +
...
ggplot(lc_raw, aes(x = dosage, y = survival_status)) + 
  ...
  geom_hline(yintercept = 0.75, 
             ...
             linetype = "longdash", 
             size = 1) +
  geom_vline(xintercept = 12.21458,
            color = "blue", 
             linetype = "longdash", 
             size = 1) +
