...
```{r}
# load average daily fecundity data
load("data/total_fecundity.Rda")

# get sample size (n), mean, standard deviation (sd), and standard error (se) for average fecundity by generation and treatment
total_fecundity <- total_fecundity %>%
  group_by(adult_generation, background) %>% 
  summarize(n = n(),
...
ggplot(total_fecundity, aes(x = background, y = mean)) +
  ...
  labs(x = "Background", y = "Mean lifetime fecundity", color = "Parental Background", fill = "Parental treatment") +
...
f0_plot <- ggplot(f0, aes(x = days_since_oviposition, y = mean)) +
  ...
  labs(y = "Mean fecundity", color = "Parental treatment", fill = "Parental treatment") +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
  theme(plot.margin = margin(20,20,20,20))
