...
### STEP 1: Calculate summary statistics for fecundity data.
In this step, we calculate mean, standard deviation, and variance for average fecundity for all experimental groups.

#### 1a: Calculate summary statistics for average daily fecundity; run the Rmd chunk below.
Here, we calculate mean, standard deviation, and variance for average daily fecundity (i.e., average number of eggs per day across lineages).

##### Calculate summary statistics for average daily fecundity data:
```{r}
# load average daily fecundity data
...
fecundity %>%
  ...
  summarize(average_fecundity = mean(number_eggs)) %>%
  group_by(adult_generation, background) %>%
  summarize(n = n(),
            mean = mean(average_fecundity),
            sd = sd(average_fecundity),
            se = sd/sqrt(n()))
```

#### 1b: Calculate summary statistics for average total fecundity; run the Rmd chunk below.
Here, we calculate mean, standard deviation, and variance for average total fecundity (i.e., average lifetime fecundity across lineages).

##### Calculate summary statistics for average total fecundity data:
```{r}
# load average daily fecundity data
load("data/fecundity.Rda")
...
# get sample size (n), mean, standard deviation (sd), and standard error (se) for average fecundity by generation and treatment
fecundity <- fecundity %>%
  filter(ifelse(adult_generation == "F0", days_since_oviposition < 20, FALSE) | 
           ifelse(adult_generation == "F1", days_since_oviposition < 10, FALSE) |
           ifelse(adult_generation == "F2", days_since_oviposition < 16, FALSE) |
           ifelse(adult_generation == "F3", days_since_oviposition < 15, FALSE)) %>%
  filter(pair_id != "L3_F1_E") %>%
  filter(pair_id != "L8_F2_E") %>%
  filter(pair_id != "L7_F3_C") %>%
  group_by(adult_generation, lineage, background) %>%
  summarize(total_number_eggs = sum(number_eggs))
```

### STEP 2: Test for correlation between adult mass and fecundity.
In this step, we determine whether there is a significant correlation between adult mass and fecundity (i.e., number of eggs laid) to see if we may need to account for mass in our model of fecundity. We might expect mass to be positively correlated with number of eggs due to greater potential for allocation of resources to reproduction in larger individuals. If a significant correlation exists between adult mass and average daily fecundity, we will need to account for it in our downstream models. First, we determine whether mass data are fitted to a normal distribution. Next, we assess the correlation between mass and mean daily fecundity based on the appropriate underlying distribution.

#### 2a: Test fit of mass and average fecundity data to a normal distribution.

##### 2a.1. Test fit of mass and average fecundity data to a normal distribution; run the Rmd chunk below.
...
df.residual(f_mean) # get degrees of freedom
...
df.residual(m_mean) # get degrees of freedom
```
...
##### 2b.2. Plot correlations for mass and average fecundity; run the Rmd chunk below.
...
# plot female mass and fecundity plot
f_plot <- ggplot(fecundity, aes(x = `F`, y = mean)) + 
  geom_smooth(method = "lm", show.legend = T, se = T, fullrange = T, color = "black") +
  geom_point(show.legend = T, size = 4, pch = 21, fill = "grey", color = "black") + 
  theme_set(theme_cowplot(12)) +
...
# plot male mass and fecundity plot 
m_plot <- ggplot(fecundity, aes(x = M, y = mean)) + 
  geom_smooth(method = "lm", show.legend = T, se = T, fullrange = T, color = "black") +
  geom_point(show.legend = T, size = 4, pch = 21, fill = "grey", color = "black") +  
  theme_set(theme_cowplot(12)) +
  scale_x_continuous(breaks = seq(from = 0, to = 160, by = 30),
...
jpeg("figures/mass_fecundity_correlation.jpeg", width = 20000, height = 8000, res = 1500)
...
In this step of the analyze, we assess holistic differences in daily fecundity (i.e., differences in trends in number of eggs laid since first oviposition) between control and exposed beetles in each generation.
...
Since 8/8 tests indicate a deviation from a normal distribution, we test the fit of multiple alternative distributions to determine whether there is consistent best-fitting distribution for the data across groups of interest. 
...
f0_mod <- glmm.zinb(number_eggs ~ days_since_oviposition*background,
                    data = f0,
                    random = ~1|pair_id,
                    correlation = corAR1(form = ~ days_since_oviposition))

# Run general linear mixed effect model for F1
f1_mod <- glmm.zinb(number_eggs ~ days_since_oviposition*background,
                    data = f1,
                    random = ~1|pair_id,
                    correlation = corAR1(form = ~ days_since_oviposition))

# Run general linear mixed effect model for F2
f2_mod <- glmm.zinb(number_eggs ~ days_since_oviposition*background,
                    data = f2,
                    random = ~1|pair_id,
                    correlation = corAR1(form = ~ days_since_oviposition))

# Run general linear mixed effect model for F3
f3_mod <- glmm.zinb(number_eggs ~ days_since_oviposition*background,
                    data = f3,
                    random = ~1|pair_id,
                    correlation = corAR1(form = ~ days_since_oviposition))

# summarize f0 model
summary(f0_mod) # get all stats

# summarize f1 model
summary(f1_mod) # get all stats

# summarize f2 model
summary(f2_mod) # get all stats

# summarize f3 model
summary(f3_mod) # get all stats
```

<b>Analysis results:</b> <br>

<b>Coefficients of general linear mixed model with autocorrelation by generation:</b><br>
<b>F0</b>: <br>
Days: t = 0.41509; p = 0.6783; d.f. = 437 <br>
Background: t = 0.14683; p = 0.8851; d.f. = 16 <br>
Days x Background : t = 0.23690; p = 0.8128; d.f. = 437 <br>

<b>F1</b>: <br>
Days: t = -0.74264; p = 0.4583; d.f. = 313 <br>
Background: t = -0.64867; p = 0.5264; d.f. = 15 <br>
Days x Background : t = 1.37520; p = 0.1701; d.f. = 313 <br>

<b>F2</b>: <br>
Days: t = -1.35692; p = 0.1757; d.f. = 353 <br>
Background: t = 1.33624; p = 0.2014; d.f. = 15 <br>
Days x Background: t = 0.32203; p = 0.7476; d.f. = 353 <br>

<b>F3</b>: <br>
Days: t = -1.885579; p = 0.0602; d.f. = 323 <br>
Background: t = -0.425972; p = 0.6771; d.f. = 13 <br>
Days x Background: t = 0.350417; p = 0.7263; d.f. = 323 <br>

These data are presented in Table 2a of the final ms.

#### 1b: Plot daily fecundity for each generation; run the Rmd chunk below.

##### Plot daily fecundity: 1) `figures/daily_fecundity_means.pdf`
```{r}
# load fecundity data
load("data/fecundity.Rda")

# group data by generation, background, and days since oviposition and summarize mean and se of daily fecundity across lineages.
daily <- fecundity %>%
...
# plot f0 daily fecundity data
f0_daily_plot <- ggplot(f0_daily, aes(x = days_since_oviposition, y = mean)) +
  ...
  labs(y = str_wrap("Mean daily fecundity", width = 10), color = "Parental treatment", fill = "Parental treatment") +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
  theme(plot.margin = margin(20,20,20,20))

# plot f1 daily fecundity data
f1_daily_plot <- ggplot(f1_daily, aes(x = days_since_oviposition, y = mean)) +
  geom_line(aes(x = days_since_oviposition, y = mean, color = background), 
            position = position_dodge(width = 0.5),
            linewidth = 2,
            show.legend = F) +
  ...
  theme(axis.title.y = element_text(vjust = +3)) + 
  theme(axis.title.x = element_blank()) +
  labs(y = str_wrap("Mean daily fecundity", width = 10)) +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
  theme(plot.margin = margin(20,20,20,20))

# plot f2 daily fecundity data
f2_daily_plot <- ggplot(f2_daily, aes(x = days_since_oviposition, y = mean)) +
  geom_line(aes(x = days_since_oviposition, y = mean, color = background), 
            position = position_dodge(width = 0.5), 
            linewidth = 2, 
            show.legend = F) +
...
# plot f3 daily fecundity data
f3_daily_plot <- ggplot(f3_daily, aes(x = days_since_oviposition, y = mean)) +
  geom_line(aes(x = days_since_oviposition, y = mean, color = background), 
            position = position_dodge(width = 0.5), 
            linewidth = 2, 
            show.legend = F) +
  ...
  theme(axis.title.y = element_text(vjust = +3)) + 
  theme(axis.title.x = element_text(vjust = -0.5)) +
  labs(x = "Time since first oviposition (days)", y = str_wrap("Mean daily fecundity", width = 10)) +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
  theme(plot.margin = margin(20,20,20,20))
...
f0_daily_plot + f1_daily_plot + f2_daily_plot + f3_daily_plot + plot_layout(nrow = 4, ncol = 1)

dev.off()
```

### STEP 2: Analyze differences in total (lifetime) fecundity between control and exposed beetles in each generation.
In this step of the analyze, we assess holistic differences in total "lifetime" fecundity (i.e., the sum of all eggs laid across days since oviposition) between control and exposed beetles in each generation.

#### 2a: Test fit of empirical data to a normal distribution; run the Rmd chunk below.
In this step, we use the shapiro-wilk test to assess whether total fecundity data in each group of interest (generation and background treatment) deviates from a normal distribution.
...
fecundity %>%
  group_by(adult_generation, background) %>%
  shapiro_test(total_number_eggs) %>%
  mutate(alpha = 0.006) %>%
  mutate(deviates = ifelse(p < alpha, "yes", "no"))
```
...
<b>Shapiro-wilk p-values for each treatment and generation:</b><br>
<b>F0_C: p = 0.79797358 </b><br>
<b>F0_E: p = 0.06929650 </b><br>
<b>F1_C: p = 0.08164203 </b><br>
<b>F1_E: p = 0.35830382 </b><br>
<b>F2_C: p = 0.90914699 </b><br>
<b>F2_E: p = 0.31504175 </b><br>
<b>F3_C: p = 0.55755291 </b><br>
<b>F3_E: p = 0.47223266 </b><br>

Since 0/8 tests indicate a deviation from a normal distribution, we assume these data are best fit by a normal distribution for the data across groups of interest. 

#### 2b: Plot empirical data distribution; run the Rmd chunk below.

##### Plot empirical data distribution for total fecundity: `figures/total_fecundity_distribution.jpeg`
```{r}
# load pupa mass data
load("data/fecundity.Rda")
...
# plot distribution
jpeg("figures/total_fecundity_distribution.jpeg", width = 5000, height = 4000, res = 500)

ggplot(fecundity, aes(x = total_number_eggs)) +
  geom_histogram(binwidth = 50, color = "black", fill = "grey", linewidth = 1) +
  theme_set(theme_cowplot(12)) +
  scale_x_continuous(breaks = seq(from = 0, to = 1200, by = 200), 
                     limits = c(0,1200)) +
...
#### 2c: #### 1a: Run general linear mixed effect autocorrelation model for each generation; run the Rmd chunk below.
In this step, we run linear mixed effect models to assess the effect of background treatment on total fecundity, with lineage as a random effect.

##### Run linear mixed effect models for total fecundity:
```{r}
# load fecundity data
load("data/fecundity.Rda")
...
f0_mod <- lmer(total_number_eggs ~ background + (1|lineage),
               data = f0)

# f1 linear mixed effect model
f1_mod <- lmer(total_number_eggs ~ background + (1|lineage),
               data = f1)

# f2 linear mixed effect model
f2_mod <- lmer(total_number_eggs ~ background + (1|lineage),
               data = f2)

# f2 linear mixed effect model
f3_mod <- lmer(total_number_eggs ~ background + (1|lineage),
               data = f3)
...
<b>F0</b>: <br>
Background: F = 0.1078; p = 0.7427 <br>
d.f. = 14 <br>

<b>F1</b>: <br>
Background: F = 0.1694; p = 0.6806 <br>
d.f. = 12 <br>

<b>F2</b>: <br>
Background: F = 3.1384; p = 0.07647 <br>
d.f. = 12 <br>

<b>F3</b>: <br>
Background: F = 2.0713; p = 0.1501 <br>
d.f. = 10 <br>

These data are presented in Table XXX of the final ms.

#### 2d: Plot total fecundity for each generation; run the Rmd chunk below.

##### Plot total fecundity: 1) `figures/total_fecundity_means.pdf`
```{r}
# load fecundity data 
load("data/fecundity.Rda")

# group data by generation, background, and days since oviposition and summarize mean and se of total fecundity across lineages.
total <- fecundity %>%
  filter(ifelse(adult_generation == "F0", days_since_oviposition < 20, FALSE) | 
           ifelse(adult_generation == "F1", days_since_oviposition < 10, FALSE) |
           ifelse(adult_generation == "F2", days_since_oviposition < 16, FALSE) |
  ...
  group_by(adult_generation, background) %>% 
  summarize(n = n(),
            mean = mean(total_number_eggs),
            sd = sd(total_number_eggs),
            se = sd/sqrt(n()))
...
# plot f0 total fecundity data
f0_total_plot <- ggplot(f0_total, aes(x = background, y = mean)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se, color = background), 
  ...
  scale_color_manual(values = c("deepskyblue2", "firebrick3")) +
  scale_fill_manual(values = c("deepskyblue2", "firebrick3")) +
  scale_y_continuous(breaks = seq(from = 0, to = 800, by = 200),
                     limits = c(0,800)) +
  theme(axis.text = element_text(size = 30)) + 
  theme(axis.title = element_text(size = 30)) + 
  theme(axis.title.y = element_text(vjust = +3)) + 
  theme(axis.title.x = element_blank()) +
  labs(x = "Parental treatment", y = str_wrap("Mean total fecundity", width = 10)) +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
  theme(plot.margin = margin(20,20,20,20))

# plot f1 total fecundity data
f1_total_plot <- ggplot(f1_total, aes(x = background, y = mean)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se, color = background), 
                position = position_dodge(width = 0.4), 
                width = 0.2, 
                linewidth = 2, 
...
# plot f2 total fecundity data
f2_total_plot <- ggplot(f2_total, aes(x = background, y = mean)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se, color = background), 
                position = position_dodge(width = 0.4), 
                width = 0.2, 
                linewidth = 2, 
...
# plot f3 total fecundity data
f3_total_plot <-ggplot(f3_total, aes(x = background, y = mean)) +
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se, color = background), 
                position = position_dodge(width = 0.4), 
                width = 0.2, 
                linewidth = 2, 
...
# plot average daily fecundity
jpeg("figures/total_fecundity_means.jpeg", width = 10000, height = 30000, res = 1700)

f0_total_plot + f1_total_plot + f2_total_plot + f3_total_plot + plot_layout(nrow = 4, ncol = 1)
