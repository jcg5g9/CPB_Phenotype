...
#### 1e: Assess phenotypic response to insecticide stress in total distance walked; run the Rmd chunk below.
...
#### 1f: Plot transformed data for total distance walked to visualize phenotypic response; run the Rmd chunk below.

##### Plot transformed data for phenotypic response analysis (for both control and exposed background samples): 1. `figures/distance_response_control.jpeg`; 2.`figures/distance_response_exposed.jpeg`
```{r}
# load distance mobility data
load("data/distance.Rda")
...
jpeg("figures/distance_response_exposed.jpeg", width = 10000, height = 8000, res = 700)
 
ggplot(distance_e, aes(x = larval_generation, y = mean, fill = treatment)) + 
  geom_bar(position = position_dodge(), stat = "identity", show.legend = T, color = "black", linewidth = 1.5) + 
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.4, position = position_dodge(0.9), linewidth = 1.5) + 
  theme_set(theme_cowplot(12)) +
  scale_fill_manual(values = c("deepskyblue2", "firebrick3")) +
