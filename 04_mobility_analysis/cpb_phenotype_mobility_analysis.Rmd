...
In this step, we plot the raw data distribution and test the fit of three potential distributions that commonly underlie continuous, non-proportion data (excluding negative poisson, binomial, and negative binomial distributions, given that they are typically used to describe count data), and determine the best fitting distribution for each group of interest. We specifically test uniform, normal, and Gamma distributions. 
...
stats <- cbind(distribution = c("uniform", "normal", "gamma"), 
               stats_1, 
               stats_2, 
               stats_3, 
               stats_4,
...
| Group     | Uniform          | Normal           | Gamma              |
|:----------|:-----------------|:-----------------| :----------------- |
| F0_C_N    | 799.661          | 793.081          | <b>773.426</b>     |
| F0_E_N    | 786.364          | 772.219          | <b>690.723</b> 	   |
| F1_C_N    | 359.610          | 359.847          | <b>350.058</b>     |
| F1_C_R    | 307.837          | 311.046          | <b>291.274</b>     |
| F1_E_N    | 293.941          | 288.267          | <b>275.932</b>     |
| F1_E_R    | 359.440          | 344.655          | <b>307.274</b>     |
| F2_C_N    | <b>238.031</b>   | 238.508          | 240.381            |
| F2_C_R    | 231.372          | 239.144          | <b>224.561</b>     |
| F2_E_N    | <b>170.610</b>   | 177.302          | 172.392            | 
| F2_E_R    | <b>148.714</b>   | 157.031          | 159.897            |
| F3_C_N    | 188.324          | 185.693          | 177.734            |
| F3_C_R    | 203.739          | 208.776          | <b>199.496</b>     |
| F3_E_N    | <b>204.908</b>   | 209.253          | 205.439            |
| F3_E_R    | 253.900          | 253.838          | <b>238.391</b>     |
| Global    | 5006.063         | 4602.635         | <b>4352.019</b>    |

The data in the majority of groups were best fit to a Gamma distribution, but the results were inconsistent across all groups, where some were best fit by a uniform distribution (e.g., F2_C_N, F2_E_N, F2_E_R, and F3_E_N). The best fitting distribution to the global dataset across groups is the Gamma distribution. We opted to assume a Gamma distribution in all general linear model analyses given the majority and since we would not expect a uniform or exponential distribution for these data.
...
# summarize f2 model
summary(f2_mod) # get z and p values
...
# summarize f3 model
summary(f3_mod) # get z and p values
...
#### 4b: Run individual logistic regression models to assess probability of mobility and estimate marginal means within treatments and backgrounds; run the Rmd chunk below.

##### Run logisitic regression models for mobility and extract marginal means:
```{r}
# load mobility data
load("data/mobility.Rda")
...
f0_mod <- glmer(moved ~ background + (1|lineage),
               family = binomial(link = "logit"),
               data = f0)

# f1 general linear mixed effect model
f1_mod <- glmer(moved ~ treatment*background + (1|lineage),
                family = binomial(link = "logit"),
                data = f1)

# f2 general linear mixed effect model
f2_mod <- glmer(moved ~ treatment*background + (1|lineage),
                family = binomial(link = "logit"),
                data = f2)

# f2 linear mixed effect model
f3_mod <- glmer(moved ~ treatment*background + (1|lineage),
                family = binomial(link = "logit"),
                data = f3)
...
jpeg("figures/mobility_probability.jpeg", width = 20000, height = 4000, res = 1000)
...
f0_plot <- ggplot(f0, aes(x = background, y = mean, group = background, color = background)) +
  ...
  scale_color_manual(values = c("deepskyblue2", "firebrick3")) +
  scale_fill_manual(values = c("deepskyblue2", "firebrick3")) +
  scale_y_continuous(breaks = seq(from = 0, to = 180, by = 30),
                     limits = c(0,180)) +
  theme(axis.text.x = element_text(angle = 0, hjust = 1)) + 
  theme(axis.text = element_text(size = 30, hjust = 0.4)) + 
  theme(axis.title = element_text(size = 30, hjust = 0.5)) + 
...
jpeg("figures/mobility_means.jpeg", width = 20000, height = 4000, res = 1000)
