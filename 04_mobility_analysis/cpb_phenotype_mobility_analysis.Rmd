...
ggplot(hn_memory, aes(x = larval_generation, y = mean, group = background)) + 
  ...
  labs(x = "Larval generation", y = "Square root of distance (mm)", fill = "Background", color = "Background") +
...
##### 1a.1. Simulate and visualize data for H<sub>a</sub>; run the Rmd chunk below.
...
ggplot(distance, aes(x = larval_generation, y = mean, group = treatment)) + 
  ...
  theme(legend.box.background = element_rect(color = "black", size = 1.5)) +
  theme(legend.box.margin = margin(5, 10, 1, 10)) +
  labs(x = "Larval generation", y = "Square root of distance (mm)", fill = "Treatment", color = "Treatment") +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
  theme(plot.margin = margin(15,15,15,15))
