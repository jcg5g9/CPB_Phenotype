...
## PHASE 1: DETERMINATION OF SUB-LETHAL (LD<sub>25</sub>) INSECTICIDE DOSAGE
In this phase of analysis, we use 48-hour survival data from a insecticide dosage experiment (see `cpb_tei_experimental_protocol.Rmd`, Section 3) conducted on third-instar larvae collected from the VTM CPB colony (see `cpb_tei_experimental_protocol.Rmd`, Section 1, for description of colony origins and laboratory maintenance). Briefly, we used a logistic regression analysis framework with a probit link function to assess the probability of survival of third-instar larvae at different pre-set concentrations of insecticide (i.e., imidacloprid).

We conducted the LD assay (as described in the experimental protocol) over multiple days over the course of two months, depending on the availability of third-instar larvae in the VTM laboratory colony. We therefore randomized all individual larvae over dosage treatments and over multiple 6-well assay plates. Specifically, we generated a list of individual larva IDs to reflect colony, dosage treatment received, and a unique numerical identifier,  based on the number of samples needed per dosage treatment. These IDs were then randomized onto 50 6-well plates.
...
<b>Summary results</b>:
LD<sub>25</sub> (initial estimate) = 10^0.9042961-ppm = ~8.02-ppm
...
jpeg("figures/ld.jpeg", width = 12000, height = 8000, res = 1500)
...
ggplot(ld_probit, aes(x = log_dosage, y = survival_status)) + 
  ...
  labs(x = "Log(concentration, ppm)", y = "Survival proportion") +
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
