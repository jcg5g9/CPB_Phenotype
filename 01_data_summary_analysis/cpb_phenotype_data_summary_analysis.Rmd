...
## PHASE 1: DATA PREPARATION AND FILTERING
In this phase of the analysis, we read in and clean the raw metadata and phenotype data for all samples and generate separate datasets for each phenotypic metric of interest, along with a complete, clean dataset with all metrics for multivariate analysis.

### STEP 1: Read in and clean data; run the Rmd chunk below.
In this step, we read in and merge our metadata with our raw phenotypic data and clean for downstream filtering and data organization steps.
...
# read in raw phenotype data 
phenotype <- read_excel("../raw_data/cpb_tei_phenotype_data.xlsx")

# get only phenotypically assayed individuals from metadata
metadata <- metadata %>%
  filter(type == "assayed")

# merge metadata and phenotype data
phenotype <- merge(metadata,
                   phenotype,
                   by = c("ind_id","generation","lineage","treatment","sex"))
...
# save complete phenotype data
save(phenotype, file = "data/full_phenotype.Rda")
```
