...
# get number of data points per mated pair across the full experiment and omit pairs with low outliers (pairs with less than 2 days of fecundity data)
fecundity_days <- fecundity %>%
  group_by(pair_id, treatment, adult_generation) %>%
  summarize(mean = mean(number_eggs),
            n = n()) %>%
  filter(n > 10)

ggplot(fecundity_days, aes(x = n, y = mean, color = treatment)) + geom_point() + geom_smooth(method = "lm")
...
<b><i>N</i><sub>data</sub> = 1,756 </b><br>
