...
# convert characters to factors in metadata and separate lineage column into "lineage" and "background" columns to indicate which treatment was administered to the F0 generation
metadata <- metadata %>%
  mutate(ind_id = factor(ind_id),
         ...
         sex = factor(sex)) %>%
  separate(lineage, c("lineage", "background"), sep = "_")
...
In this step, we read in and clean the full performance dataset (`../raw_data/performance_data.xlsx`). We then merge the performance data with sample metadata and separate into distinct datasets for each performance metric of interest. Data (type "assayed", see metadata info in Step 1a above) include: <br>
   ...
   17-53. "...hr_status"</b>: a binary indicator of whether the beetle was alive (i.e., "1") or dead (i.e., "0") after each 24-hour interval over 888 hrs total. Note: individuals were marked with "0" for all days following storage in the freezer.
   ...
   59. <b>"deformity"</b>: a binary indicator of whether the adult beetle exhibited developmental abnormalities/deformities upon emergence (i.e., "1") or appeared developmentally normal (i.e., "0")
...
# convert characters to factors (categorical variables) and numerics (numeric variables) in performance data, eliminate L4 date column, separate lineage column into "lineage" and "background" columns to indicate which treatment was administered to the F0 generation
...
In this step, we get only larval mobility metrics of interest (i.e., total distance walked, average walking rate, and average walking acceleration) and filter and clean the mobility dataset.
...
In this step, we get only the larval herbivory metric of interest (i.e., total leaf tissue consumed) and filter and clean the herbivory dataset.
...
In this step, we get only larval development metrics of interest (i.e., time to pupation, time from pupae to adult emergence, pupal mass, and adult mass) and filter and clean the development dataset.
...
Here, we summarize all fecundity (i.e., egg count) data from the experiment.
...
# get number of fecundity data points by generation and treatment
fecundity %>%
...
<b><i>N</i><sub>data</sub> = 1,501 </b><br>

<b>Sample size per generation:</b><br>
<b><i>N</i><sub>F0</sub> = 457 </b><br>
<b><i>N</i><sub>F1</sub> = 332 </b><br>
<b><i>N</i><sub>F2</sub> = 372 </b><br>
<b><i>N</i><sub>F3</sub> = 340 </b><br>

<b>Sample size per treatment per generation:</b><br>
<b><i>N</i><sub>F0_C</sub> = 250 </b><br>
<b><i>N</i><sub>F0_E</sub> = 207 </b><br>
<b><i>N</i><sub>F1_C</sub> = 224 </b><br>
<b><i>N</i><sub>F1_E</sub> = 108 </b><br>
<b><i>N</i><sub>F2_C</sub> = 241 </b><br>
<b><i>N</i><sub>F2_E</sub> = 131 </b><br>
<b><i>N</i><sub>F3_C</sub> = 201 </b><br>
<b><i>N</i><sub>F3_e</sub> = 139 </b><br>
...
##### 2b.1. Summarize mobility data; run the Rmd chunk below.
...
<b><i>N</i><sub>F0_C_C</sub> = 67 </b><br>
<b><i>N</i><sub>F0_C_E</sub> = 70 </b><br>

<b><i>N</i><sub>F1_C_C</sub> = 30 </b><br>
<b><i>N</i><sub>F1_C_E</sub> = 27 </b><br>
<b><i>N</i><sub>F1_E_C</sub> = 23 </b><br>
<b><i>N</i><sub>F1_E_E</sub> = 28 </b><br>

<b><i>N</i><sub>F2_C_C</sub> = 21 </b><br>
<b><i>N</i><sub>F2_C_E</sub> = 23 </b><br>
<b><i>N</i><sub>F2_E_C</sub> = 15 </b><br>
<b><i>N</i><sub>F2_E_E</sub> = 14 </b><br>

<b><i>N</i><sub>F3_C_C</sub> = 16 </b><br>
<b><i>N</i><sub>F3_C_E</sub> = 19 </b><br>
<b><i>N</i><sub>F3_E_C</sub> = 18 </b><br>
<b><i>N</i><sub>F3_E_E</sub> = 23 </b><br>

##### 2b.2. Summarize herbivory data; run the Rmd chunk below.
...
<b>Sample size per background (F0 treatment) and treatment (assay treatment) per generation:</b><br>
<b><i>N</i><sub>F0_C_C</sub> = 72 </b><br>
<b><i>N</i><sub>F0_C_E</sub> = 72 </b><br>

<b><i>N</i><sub>F1_C_C</sub> = 31 </b><br>
<b><i>N</i><sub>F1_C_E</sub> = 30 </b><br>
<b><i>N</i><sub>F1_E_C</sub> = 30 </b><br>
<b><i>N</i><sub>F1_E_E</sub> = 29 </b><br>

<b><i>N</i><sub>F2_C_C</sub> = 22 </b><br>
<b><i>N</i><sub>F2_C_E</sub> = 23 </b><br>
<b><i>N</i><sub>F2_E_C</sub> = 17 </b><br>
<b><i>N</i><sub>F2_E_E</sub> = 17 </b><br>
...
# isolate data to calculate time to pupation (pupa date only), including all sample metadata
pupa_time <- development %>%
  select(ind_id:sex, assay_date:molt, pupa_date) %>%
  drop_na(pupa_date)

# isolate data to calculate pupation duration (pupa date and emergence data), including all sample metadata
...
##### 2b.3.2. Summarize time to pupation data; run the Rmd chunk below.
Here, we summarize data for time to pupation.
...
# get total development metric data points
development %>%
  count()

# get number of development data points by larval generation
development %>%
  group_by(larval_generation) %>%
  count()

# get number of development data points by background and treatment in each larval generation
development %>%
  group_by(larval_generation, background, treatment) %>%
  count()
```

##### 2b.3.3. Summarize pupation duration data; run the Rmd chunk below.
Here, we summarize data for duration of pupation.
...
##### 2b.3.4. Summarize pupa mass data; run the Rmd chunk below.
Here, we summarize data for pupa mass.

##### Summarize pupa mass data:
```{r}
# load pupa mass data
load("../06_development_analysis/data/pupa_mas.Rda")
...
##### 2b.3.5. Summarize adult mass data; run the Rmd chunk below.
Here, we summarize data for adult mass.

##### Summarize adult mass data:
```{r}
# load adult mass data
load("../06_development_analysis/data/adult_mas.Rda")
```
