...
fecundity <- fecundity %>%
  dplyr::select(-c(larval_generation)) %>%
  drop_na(number_eggs) %>%
...
In this step, we get only larval mobility metrics of interest (i.e., total distance walked, average walking rate, and average walking acceleration) and filter and clean the mobility dataset. Additionally, we generate a new variable (additional column) in the dataset (i.e., "moved") to represent whether a larva moved at all (i.e., if total distance was non-zero and therefore the larva moved, the individual was assigned a value of "1"; if total distance was zero and therefore the larva did not move, the individual was assigned a value "0"). The dataset is then saved to the corresponding analysis directory (`04_mobility_analysis`).
...
# get only mobility data from performance data, add new column indicating whether an individual moved or not (had zero or non-zero value for total distance), and drop na data in the metrics columns (average speed through total distance)
mobility <- performance %>%
  dplyr::select(c(ind_id:type, larval_generation:molt, ave_speed:total_distance)) %>%
  mutate(moved = as.numeric(ifelse(total_distance != 0, "1", "0"))) %>%
...
In this step, we get only the larval herbivory metric of interest (i.e., total leaf tissue consumed) and filter and clean the herbivory dataset. Additionally, we generate a new variable (additional column) in the dataset (i.e., "consumed") to represent whether any leaf tissue was consumed at all by a beetle (i.e., if leaf area consumed was non-zero and therefore some tissue was eaten, the individual was assigned a value of "1"; if leaf area consumed was zero and therefore no tissue was eaten, the individual was assigned a value "0"). The dataset is then saved to the corresponding analysis directory (`05_herbivory_analysis`).
...
# get only herbivory data from performance data and add binary value indicating whether there was consumption (had zero or non-zero value for leaf_area_consumed)
herbivory <- performance %>% 
  dplyr::select(c(ind_id:percent_leaf_consumed)) %>%
...
development <- performance %>%
  dplyr::select(c(ind_id:type, larval_generation:molt, pupa_date:stored_date))

# save development data
save(development, file = "../06_development_analysis/data/development.Rda")
```

##### 1c.4. Isolate, filter, and clean survival data from performance data; run the Rmd chunk below.
In this step, we get only survival-related data from the development dataset and filter and clean the surival dataset. The dataset is then saved to the corresponding analysis directory (`06_development_analysis`).
...
# get only survival data from performance data
survival <- performance %>%
  dplyr::select(c(ind_id:type, larval_generation:molt, emerge_date)) %>%
  mutate(survived = as.numeric(ifelse(is.na(emerge_date), "0", "1")))

# save survival data
save(survival, file = "../07_survival_analysis/data/survival.Rda")
```
...
# save daily fecundity data for downstream analyses
save(daily_fecundity, file = "../03_fecundity_analysis/data/daily_fecundity.Rda")

# save total fecundity data for downstream analyses
save(total_fecundity, file = "../03_fecundity_analysis/data/total_fecundity.Rda")
```
...
These data are presented in Table 1a of the final ms.

##### 2a.2. Summarize total fecundity data; run the Rmd chunk below.
Here, we summarize total data. To assess total "lifetime" fecundity without bias, it is necessary to total the number of eggs over the same number of days for each mated pair within background treatment, because each pair must have had the opportunity to produce eggs for the exact same amount of time. Although we recorded fecundity daily for each pair across all generations, some individual beetles in some pairs died before others within generations, and those pairs were therefore omitted from further egg counts. For this reason, within each generation, we first determined the maximum number of days at which the highest possible number of pairs were still laying eggs, and we then totaled number of eggs for each pair up to this day threshold. 

##### Summarize total fecundity data:
```{r}
# load fecundity data
load("../03_fecundity_analysis/data/fecundity.Rda")

# Determine maximum number of days to use for pairs within each generation for calculating total fecundity.
fecundity %>%
  group_by(adult_generation, days_since_oviposition) %>%
  summarize(n = n())

# generate dataset with number of days since oviposition for each generation determined from code above, and omit any mated pairs falling below the threshold for each generation
fecundity <- fecundity %>%
...
# calculate average total eggs for each background, lineage, and generation and make new dataframe
total_fecundity <- fecundity %>%
  group_by(adult_generation, lineage, background) %>%
...
<b>Total sample size:</b><br>
<b><i>N</i><sub>data</sub> = 64 </b><br>

<b>Sample size per generation:</b><br>
<b><i>N</i><sub>F0</sub> = 18 </b><br>
<b><i>N</i><sub>F1</sub> = 16 </b><br>
<b><i>N</i><sub>F2</sub> = 16 </b><br>
<b><i>N</i><sub>F3</sub> = 14 </b><br>

<b>Sample size per background per generation:</b><br>
<b><i>N</i><sub>F0_C</sub> = 10 </b><br>
<b><i>N</i><sub>F0_E</sub> = 8 </b><br>
<b><i>N</i><sub>F1_C</sub> = 10 </b><br>
<b><i>N</i><sub>F1_E</sub> = 6 </b><br>
<b><i>N</i><sub>F2_C</sub> = 10 </b><br>
<b><i>N</i><sub>F2_E</sub> = 6 </b><br>
<b><i>N</i><sub>F3_C</sub> = 8 </b><br>
<b><i>N</i><sub>F3_e</sub> = 6 </b><br>
...
<b><i>N</i><sub>F0_E_N</sub> = 72 </b><br>
...
<b><i>N</i><sub>F0_E_N</sub> = 70 </b><br>
...
pupa_time <- development %>%
  dplyr::select(ind_id:sex, assay_date:molt, pupa_date) %>%
  drop_na(pupa_date) %>%
...
pupa_duration <- development %>%
  dplyr::select(ind_id:sex, assay_date:molt, pupa_date, emerge_date) %>%
...
pupa_mass <- development %>%
  dplyr::select(ind_id:sex, assay_date:molt, pupa_mass) %>%
  drop_na(pupa_mass)

# isolate adult mass data, including all sample metadata
adult_mass <- development %>%
  dplyr::select(ind_id:sex, assay_date:molt, adult_mass) %>%
  drop_na(adult_mass)
...
<b>Sample size per background (F0 treatment) and treatment (assay treatment) per generation:</b><br>
<b><i>N</i><sub>F0_C_N</sub> = 67 </b><br>
<b><i>N</i><sub>F0_E_N</sub> = 42 </b><br>
...
<b><i>N</i><sub>F0_E_N</sub> = 38 </b><br>
...
##### 2b.5. Summarize survival data; run the Rmd chunk below.
Here, we summarize survival, including the number of individuals across the entire dataset. 

##### Summarize survival data:
```{r}
# load survival data
load("../07_survival_analysis/data/survival.Rda")

# get total survival metric data points
survival %>%
  count()

# get number of survival data points by larval generation
survival %>%
  group_by(larval_generation) %>%
  count()

# get number of survival data points by background and treatment in each larval generation
survival %>%
  group_by(larval_generation, background, treatment) %>%
  count()
```
