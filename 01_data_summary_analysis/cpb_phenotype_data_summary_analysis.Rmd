...
# calculate average total fecundity for each background, lineage, and generation and make new dataframe
total_fecundity <- fecundity %>%
  filter(days_since_oviposition < 11) %>%
  group_by(adult_generation, lineage, background) %>%
  summarize(total_number_eggs = sum(number_eggs)) %>%
  as.data.frame()

fecundity %>%
  group_by(days_since_oviposition) %>%
  count()
