...
mobility <- performance %>%
  dplyr::select(c(ind_id:type, larval_generation:molt, ave_speed:total_distance), -c(pair_id:adult_generation)) %>%
...
herbivory <- performance %>% 
  dplyr::select(c(ind_id:percent_leaf_consumed), -c(pair_id:adult_generation)) %>%
...
development <- performance %>%
  dplyr::select(c(ind_id:type, larval_generation:molt, `24hr_status`:stored_date), -c(pair_id:adult_generation))
